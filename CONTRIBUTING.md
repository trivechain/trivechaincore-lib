# Contributing to trivechaincore-lib

We're working hard to make *trivechaincore-lib* the most powerful JavaScript library for working with Trivechain. Our goal is to have *trivechaincore-lib* be a library that can be used by anyone interested in Trivechain, and to level expertise differences with great design and documentation.
...
These are some global design goals in trivechaincore-lib that any change must adhere.
...
When possible, trivechaincore-lib objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of trivechaincore-lib methods. For example, calling `JSON.stringify` on an trivechaincore-lib object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'trivechaincore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the trivechaincore object.

DO:
```javascript
var trivechaincore = require('../');
var PublicKey = trivechaincore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add trivechain git@github.com:trivechain/trivechaincore-lib.git
git pull --rebase trivechain master
```
...
```
Finally go to [github.com/trivechain/trivechaincore-lib](https://github.com/trivechain/trivechaincore-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of trivechaincore-lib.

If you have any questions feel free to post them to
[github.com/trivechain/trivechaincore-lib/issues](https://github.com/trivechain/trivechaincore-lib/issues).
