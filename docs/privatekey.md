# Private Key
Represents a Trivechain private key and is needed to be able to spend funds and sign transactions. See the official [Trivechain Wiki](https://en.bitcoin.it/wiki/Private_key) for more information about private keys. A PrivateKey in trivechaincore is an immutable object that has methods to import and export into a variety of formats including [Wallet Import Format](https://en.bitcoin.it/wiki/Wallet_import_format).
...
Note: The WIF (Wallet Import Format) includes information about the network and if the associated public key is compressed or uncompressed (thus the same Trivechain address will be generated by using this format).
